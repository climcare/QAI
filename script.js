...
// RC1.4.7 HOTFIX-02
// Correção:
// Evita Race Condition empilhada de requisições de rede lentas trazidas 
// pelo setInterval puro. Converte o ciclo rígido em agendador adaptativo.
// -------------------------------------------------------------------
// AJUSTE ADICIONAL: Remoção do operador 'await' para impedir o travamento
// síncrono da inicialização da navegação do workspace.
// ===================================================================
window.onload = async () => {
    ...
    if (typeof supabase !== "undefined") {
        ...
        inicializarCacheDOM();
        executarCicloMonitoramentoProtegido();
    }
...
// ===================================================================
// RC1.4.7 HOTFIX-03 (OBRIGATÓRIO)
// Correção:
// Garante a continuidade do monitoramento através do bloco finally,
// mesmo que ocorra uma exceção inesperada no ciclo de processamento.
// ===================================================================
async function executarCicloMonitoramentoProtegido() {
    try {
        await processarCicloMonitoramento();
    } finally {
        setTimeout(executarCicloMonitoramentoProtegido, 15000);
    }
}
...
function atualizarMetadadosDispositivo(reg) {
    ...
    // ===================================================================
    // RC1.4.7 HOTFIX-01 (OBRIGATÓRIO)
    // Correção:
    // Remove a dependência de coerção booleana implícita para validação 
    // do nível de sinal (reg.signalStrength), prevenindo bugs de valor zero.
    // ===================================================================
    if (domElements.txtSignal) {
        const sinalValido =
            reg.signalStrength !== null &&
            reg.signalStrength !== undefined;

        domElements.txtSignal.textContent =
            sinalValido
                ? `${reg.signalStrength} dBm`
                : '-- dBm';
    }
...
function atualizarComponenteScore(score) {
    ...
    const scoreContainer = domElements.scoreContainer || document.getElementById('scoreContainer');
    if (scoreContainer) {
        if (!domElements.scoreContainer) domElements.scoreContainer = scoreContainer;
        scoreContainer.className = `score-orb ${corBorda} ${bgScore}`;
    }
}
...
function atualizarBarraStatusGeral(status, substatus) {
    ...
    domElements.panelStatusGeral.className = `panel status-panel ${cfg.bg} ${cfg.border}`;
...
function atualizarCardMetrica(sufixo, valor, unidade, status) {
    ...
    if (elCard) elCard.className = `metric-card ${clsCard}`;
    if (elStatus) {
...
// RC1.4.7 HOTFIX-04
// Correção:
// Padroniza a validação e a coerção de strings numéricas vindas da
// telemetria do Supabase substituindo parseFloat() por Number() e 
// protegendo a renderização contra falhas via Number.isFinite().
// ===================================================================
function renderizarPainelParticulas(reg, metricas) {
    ...
    dados.forEach(d => {
        ...
        const massaNum = Number(d.massa);
        const contaNum = Number(d.conta);

        const mVal = Number.isFinite(massaNum) ? massaNum.toFixed(1) : '--';
        const cVal = Number.isFinite(contaNum)
            ? Math.round(contaNum).toLocaleString('pt-BR')
            : '--';
...
function renderizarRecomendações(plano) {
    ...
    // RC1.4.7 HOTFIX-01
    // Correção:
    // ReferenceError local causado por atribuição circular auto-referenciada 
    // (container = container || domElements.panelTriagem).
    // ===================================================================
...
function obterTextoMitigacaoBase(param) {
    const mitigacoes = {
        ...
        Verifique a necessidade de incidência solar, a ocupação do ambiente e o funcionamento do sistema de climatização.`,
